...
def _resolve_ground_z(
    ...
    anchor_ground_z: float | None,
    config: GroundPlaneConfig,
) -> float:
    """Reconcile RANSAC ground and ground-anchor-derived ground conservatively."""
    if anchor_ground_z is None:
        return ransac_ground_z

    disagreement = abs(anchor_ground_z - ransac_ground_z)
    if disagreement > config.max_anchor_ground_disagreement_m:
        logger.warning(
            "Anchor ground Z (%.3f) and RANSAC ground Z (%.3f) disagree by %.2f m — "
    ...
    blended = float(np.median([ransac_ground_z, anchor_ground_z]))
    logger.info(
        "Anchor ground Z agrees with RANSAC within %.2f m — blending to %.3f m",
        disagreement,
        blended,
    )
    return blended
...
def _anchor_footprint_mask(
    pts_xy: np.ndarray,
    ground_anchor_positions: list[np.ndarray],
    transform_matrix: np.ndarray,
    margin_m: float,
) -> np.ndarray | None:
    """Return a mask for points inside the transformed ground-anchor footprint.

    ``ground_anchor_positions`` are XYZ points the algorithm uses as a
    low-confidence prior for ground level (e.g. ARKit horizontal-plane anchor
    centres or any operator-placed markers on the ground). When fewer than 3
    anchors are supplied, the spatial crop is skipped (returns ``None``).
    """
    if ground_anchor_positions is None or len(ground_anchor_positions) < 3:
        return None
    ...
    try:
        ...
        transformed_anchors = []
        for anchor in ground_anchor_positions:
            p = np.append(anchor, 1.0)
            transformed_anchors.append((transform_matrix @ p)[:2])

        anchor_xy = np.array(transformed_anchors)
        hull = ConvexHull(anchor_xy)
        if hull.volume < 1.0:
            logger.warning(
                "Anchor bounding area is too small (%.2f m²). Anchors may be placed in a line. "
                "Skipping spatial crop to avoid cutting off the pile.",
                hull.volume,
            )
            return None

        hull_vertices = anchor_xy[hull.vertices]
        hull_path = MplPath(hull_vertices)
        inside = hull_path.contains_points(pts_xy, radius=2 * margin_m if margin_m > 0 else 0.0)
        logger.info(
            "Anchor footprint crop: keeping %d / %d points within %.2f m margin",
            int(inside.sum()),
            len(inside),
            margin_m,
        )
        ...
        logger.warning("Could not compute anchor boundary hull: %s", e)
        return None
...
def segment_pile(
    pcd: o3d.geometry.PointCloud,
    config: GroundPlaneConfig,
    ground_anchor_positions: list[np.ndarray] | None = None,
) -> GroundPlaneResult:
    ...
    5. Optionally crop to the ground-anchor bounding polygon

    ``ground_anchor_positions`` is an optional list of XYZ points used as a
    low-confidence prior for ground level (e.g. ARKit horizontal-plane anchor
    centres or any operator-placed markers on the ground). When fewer than
    three anchors are supplied, the prior is ignored cleanly.
    """
    # 1. Remove outliers
    ...
    if ground_anchor_positions:
        ...
        for anchor_pos in ground_anchor_positions:
            ...
            anchor_zs.append(float((T @ anchor_h)[2]))
        anchor_ground_z = float(np.median(anchor_zs))
        logger.info(
            "Anchor-based ground Z: %.3f m (from %d anchors, RANSAC ground Z: %.3f)",
            anchor_ground_z, len(anchor_zs), ground_z,
        )
        ground_z = _resolve_ground_z(ground_z, anchor_ground_z, config)
    ...
    # 5. Optional: spatial crop using ground-anchor positions
    if ground_anchor_positions and len(ground_anchor_positions) >= 3:
        inside = _anchor_footprint_mask(
            pts[:, :2], ground_anchor_positions, T, config.anchor_crop_margin_m
        )
